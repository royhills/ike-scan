...
psk_crack_usage(int status) {
   ...
   fprintf(stderr, "(-P) option.  This file can contain one or more entries.  For muliple entries,\n");
   fprintf(stderr, "each one must be on a seperate line.\n");
   fprintf(stderr, "\n");
   ...
   fprintf(stderr, "seperately, so it is possible to crack a mixture of MD5 and SHA1 hashes.\n");
   fprintf(stderr, "\n");
